...
public:
	struct InventoryData
	{
		...
		void Process_Test()
		{
			...
			_MESSAGE("ARRAY SIZE: %d", array.GetArraySize());
			items.clear();
		}
	};

		//void __cdecl sub_849830(int a1, int a2, unsigned int a3, unsigned int a4)
		//{
		//	int v4; // edi@1
		...
		//		do
		//		{
		//			v5 = sub_849000((int *)a1, a2, v4, a4);
		//			v6 = v5;
		//			if (v5)
		//				sub_849830(a1, a2, v4, v5 - 1);
		//			if (v6 >= *(_DWORD *)(a1 + 8) - 1)
		//				break;
		//			v4 = v6 + 1;
		//		} while (v6 + 1 < a4);
		...
		//static void Process(BSTArray<StandardItemData*>* items, void* obj, UInt32 begIndex, UInt32 endIndex)
		//{
		//	UInt32 curIndex = begIndex;
		...
		//}
			...
			//TESObjectREFR* m_containerRef = nullptr;
			//void(__cdecl* GetContainerRef)(void*, TESObjectREFR*&) = (void(__cdecl*)(void*, TESObjectREFR*&))0x004A9180;
			//GetContainerRef((void*)0x01B3E764, m_containerRef);
			//if (m_containerRef != nullptr)
			//{
			//	UInt32& lootMode = *(UInt32*)0x01B3E6FC;
			//	UInt32 iActivateDist = g_gameSettingCollection->Get("iActivatePickLength")->data.u32 + 50;//iActivatePickLength

			//	if (settings.m_fadeOutDist >= iActivateDist && m_containerRef->Is(FormType::Character) && !m_containerRef->IsDead(false) && lootMode == 2)
			//	{
		...
		if (pargs->pThisMenu != nullptr)
		{
			...
			if (objDesc != nullptr)
			{
				...
				auto processTransfer = [=]() {
					...
					{
						//containerMenu->inventoryData->Process_Test();
						containerMenu->inventoryData->Update(g_thePlayer);
						//_MESSAGE("BEFORE:COUNT: %d", containerMenu->inventoryData->items.size());

						//for (auto& item : containerMenu->inventoryData->items)
						//{
						//	_MESSAGE("NAME: %s		COUNT: %d		HANDLE: %08X", item->GetName(), item->GetCount(), item->refHandle);
						//}
						...
						//auto inventoryData = containerMenu->inventoryData;
						//inventoryData->ui.ClearElements();

						//for (const auto& item : containerMenu->inventoryData->items)
						//{
						//	inventoryData->ui.PushBack(item->fxValue);
						//}
						//containerMenu->inventoryData->selected = false;

						//_MESSAGE("AFTER: ITEMCOUNT: %d", containerMenu->inventoryData->items.size());
